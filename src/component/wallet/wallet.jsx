...
function wallet({ role }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (
      !isString(formData.accountHolder) ||
      formData?.accountHolder?.trim()?.length < 3
    ) {
    ...
    <div className="bg-gray-300 pb-8 mt-5 rounded-md">
      ...
      {withdrawForm ? (
        <div className=" flex flex-col p-5  items-center w-2/3 m-auto rounded-md text-center bg-white">
          <div>
            <h1 className="text-gray-800 font-medium text-2xl mb-5 ">
              Withdrawal Form{" "}
            </h1>
          </div>
          <div
            className="w-4/6 h-[75%] rounded-xl bg-opacity-80 p-8"
            style={{
              background: "linear-gradient(to bottom, #5050F9, #7777D6)",
              backgroundRepeat: "no-repeat",
          ...
          >
            <form onSubmit={handleSubmit}>
              <div className="flex flex-col text-center mt-[4%] gap-y-4 items-center">
                ...
                <button
                  type="submit"
                  className="px-8 py-2 rounded-md bg-indigo-600 text-white font-bold hover:bg-blue-800 border"
                >
                  Submit
                </button>
                <button
                  onClick={() => setwithdrawForm(false)}
                  className="px-8 py-1 rounded-md bg-slate-400 text-white font-bold hover:bg-slate-500 border border-gray-300"
                >
                  Cancell
                </button>
              </div>
              ...
              filter?.map((data) => (
                ...
                >
                  ...
                  <div className="text-xs sm:text-base flex justify-around items-center col-span-7">
                    <div>
                      <h4>
                        {activeStatus === "in" ? "Received" : "Withdrawn"}
                      </h4>
