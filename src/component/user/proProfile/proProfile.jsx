...
function proProfile() {
  ...
  if (location.state === null) {
    ...
    const review = async () => {
      try {
        const data = await userAxios.get(`/getBookings?proId=${proData._id}`);
        if (data.data.orders) {
          const reviewed = data.data.orders.filter(
            (order) => order.reviews.star
          );
          let work_compleated = [];
          work_compleated = data.data.orders.filter(
            (order) => order.work_status.status == "completed"
          );
          setWorkDone(work_compleated.length);
          let workCancelledByPro = [];
          workCancelledByPro = data.data.orders.filter(
            (order) =>
              order.pro_confirmed === true &&
              order.work_status.status === "cancelled" &&
              order.work_status.role === "pro"
          );
          setWorkCancelled(workCancelledByPro.length);
    ...
    const reviews = orders.filter(
      (data) => data.reviews.description.length > 0
    );
                ...
                <div className="bg-white border border-white shadow-lg  p-4 m-4">
                  <div className=" md:flex">
                    <div className=" h-10 w-10 md:h-32 md:w-32 m-auto  sm:mb-0 mb-3">
                      <img
                        src={proData.image ? proData.image : "/icons/man.png"}
                        alt="profile"
                        className="  object-fill rounded-full w-10 h-10 md:w-24 md:h-24"
                      />
                    </div>
            ...
            <div className="w-full md:w-2/6  rounded-lg shadow-lg p-2 md:p-5">
              ...
              <div className="flex flex-wrap justify-center gap-2 mb-2">
                {proData.skills?.length > 0 ? (
                  proData.skills.slice(0, 5).map((skill) => (
                    <button
                      key={skill._id}
                      className="bg-gray-200 text-blue-600 rounded-lg px-4 py-2"
                    >
                      {skill.skill}
                    </button>
                  ))
